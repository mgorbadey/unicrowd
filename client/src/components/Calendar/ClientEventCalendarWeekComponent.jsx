...
export default function ClientEventCalendarWeekComponent({ clientSlot }) {
  ...
  return (
    <>
      <li
        className={`relative mt-px flex sm:col-start-${clientSlot.weekDay}`}
        style={{ gridRow: `${clientSlot.gridRow} / span ${clientSlot.span}` }}
      >
        <a
          href={`/masters/${clientSlot.masterId}/schedules/${clientSlot.id}`}
        ...
        >
          ...
          <button className="bg-pink-500 text-white active:bg-pink-600 font-bold uppercase text-xs  rounded-full shadow hover:shadow-md outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button"
      >
  {clientSlot.status}
</button>
